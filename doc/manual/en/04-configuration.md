...
| tls_cert_monitor_interval | 3600 | String | No | The interval between TLS certificate expiry checks. The monitor checks all configured TLS certificates (`[pgagroal]`, metrics endpoint, and per-server) and logs warnings or errors if they are expiring soon or already expired. Prometheus metrics are also updated to reflect their validity. Set to `0` to disable. If this value is specified without units, it is taken as seconds. It supports the following units as suffixes: 'S' for seconds (default), 'M' for minutes, 'H' for hours, 'D' for days, and 'W' for weeks. |
