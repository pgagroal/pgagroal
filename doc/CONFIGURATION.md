...
| tls_cert_monitor_interval | 3600s | String | No | The interval between TLS certificate metric refreshes. The monitor re-parses the configured TLS certificates (`[pgagroal]`, metrics endpoint, and per-server) so that Prometheus expiration metrics stay current when certificates are rotated on disk. Set to `0` to disable. If this value is specified without units, it is taken as seconds. It supports the following units as suffixes: 's' for seconds (default), 'm' for minutes, 'h' for hours, 'd' for days, and 'w' for weeks. |
